...
- Поддерживаемые форматы: `epub`, `fb2`, `mobi`, `prc`, `azw`, `azw3`, `txt`, `text`, `diff`, `po`, `log`, `ini`, `conf`, `rtf`, `rtx`, `pdf`, `djvu`, `djv`, `doc`, `dot`, `docx`, `cbz`, `cbr`.
- Книги и документы также принимаются в ZIP-обёртках с именами вида `книга.fb2.zip`; при отправке сохраняются содержимое и расширение файла.
...
- Проверка обновлений при открытии приложения и ручная проверка из настроек.
...
Приложение проверяет наличие новой версии при открытии и позволяет запустить ручную проверку в настройках. Перед открытием системного установщика скачанный APK проверяется на целостность и совпадение подписи с уже установленным приложением.

Если пользователь отменил системную установку, скачанный APK остается во внутреннем кеше приложения. Повторная установка использует кеш только при совпадении актуального манифеста и SHA-256; устаревшие, поврежденные или уже установленные APK удаляются. Кнопка **«Очистить кэш загрузок»** в настройках также удаляет кеш APK обновлений.
...
| --- | --- |
| Kotlin 2.3.21 | основной язык разработки |
...
| JSoup 1.23.2 | HTML/XML-парсинг для сетевых источников |
| Play Services Code Scanner | сканирование QR-кодов |
| Junrar 8.1.1 | чтение CBR/RAR-архивов |
...
eBookSender предоставляется "как есть", без каких-либо явных или подразумеваемых гарантий. Пользователь самостоятельно отвечает за соблюдение законов, наличие прав на передаваемые и скачиваемые материалы, безопасность своей сети и устройства, корректность FTP-настроек, резервное копирование данных и использование сторонних источников.

Приложение является инструментом для работы с файлами и не предоставляет прав на сторонний контент. Передача по FTP может выполняться без шифрования, если выбранное устройство или сервер не поддерживает защищенный режим.

Полный текст отказа от ответственности находится в [DISCLAIMER.md](DISCLAIMER.md).
