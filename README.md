...
- **`release.yml`** — при пуше тега `v*` (например `v0.2.0`): проверяет SemVer-формат тега, запускает unit-тесты, собирает **подписанные** release-APK для `arm64-v8a`, `armeabi-v7a` и universal-варианта, затем публикует их в GitHub Releases с автогенерацией списка изменений.
...
3. GitHub Actions проверит тег, прогонит unit-тесты, соберёт подписанные APK и опубликует их на странице Releases:

   ```text
   eBookSender-v0.2.0-universal.apk
   eBookSender-v0.2.0-arm64-v8a.apk
   eBookSender-v0.2.0-armeabi-v7a.apk
   ```
