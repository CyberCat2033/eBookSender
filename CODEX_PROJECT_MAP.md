...
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/PocketBookServerUpdateManager.kt` - PocketBook `pb-ftp` server updater; checks automatically when a PocketBook profile connects and manually from settings, reads installed version through the local control API, validates the GitHub Pages manifest and fixed launcher install path, loads localized changelog markdown, downloads/checksums the launcher artifact, stages it under `applications/.pb-ftp-update`, activates newer servers through local `POST /update`, falls back to direct launcher replacement for legacy servers, supports cancellation, progress state, and cache cleanup.
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/UpdateManifestLoader.kt` / `UpdateArtifactDownloader.kt` / `UpdateFileHelpers.kt` / `UpdateJobController.kt` - shared update infrastructure for HTTPS JSON manifest loading, artifact downloads with cancellation/progress, SHA-256 verification, safe cache file names, cache-directory cleanup, and update check/install/status-clear job lifecycle used by both app and PocketBook server updaters.
...
- `app/src/main/java/com/cybercat/ebooksender/update/AppUpdateManagerImpl.kt` - app update implementation; loads the HTTPS manifest without using the app HTTP cache, selects an ABI-compatible APK, loads localized changelog markdown into `cache/update-changelogs`, reuses verified cached APKs, downloads updates to `cache/update-apks`, removes installed/stale APKs and changelogs on app start, verifies SHA-256, package name, increasing version code, and signing certificate before launching Android's installer through `FileProvider`; if Android requires the unknown-sources permission first, `MainActivity.onResume()` asks it to show the update dialog again after permission is granted.
