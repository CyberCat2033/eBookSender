...
- `app/build.gradle.kts` - app configuration and dependencies. Auto-versioning reads `versionName`/`versionCode` from git (latest reachable release tag matching `v[0-9]*` and commit count via `providers.exec`); falls back to `0.1.0`/`1` when there are no release tags or outside a git repo. Release signing reads `RELEASE_STORE_FILE`/`RELEASE_STORE_PASSWORD`/`RELEASE_KEY_ALIAS`/`RELEASE_KEY_PASSWORD` from `local.properties`, Gradle properties, or environment variables.
