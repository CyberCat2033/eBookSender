...
- `gradle/libs.versions.toml` - dependency and plugin versions. Keep AGP 8.13.2, Kotlin 2.3.21, KSP2, and Dagger/Hilt 2.58 compatible: Hilt 2.59+ requires AGP 9. Compose BOM 2026.06.01, Core 1.17.0, Activity 1.12.4, Lifecycle 2.10.0, Navigation 2.9.8, and AndroidX Hilt 1.3.0 retain compatibility with compileSdk 36; newer releases require newer SDK/AGP via direct or transitive dependencies. Check published AAR metadata before raising these versions. Compose Hilt ViewModels use `androidx.hilt.lifecycle.viewmodel.compose.hiltViewModel` from `hilt-lifecycle-viewmodel-compose`.
...
- `.github/workflows/codeql.yml` - CodeQL advanced setup for GitHub Actions and Java/Kotlin: analyzes workflow files directly, initializes Java/Kotlin in manual build mode, sets up JDK 17 and Gradle, then traces `:app:assembleDebug` so Android debug sources are analyzed without requiring release signing secrets. GitHub default setup must stay disabled because GitHub rejects advanced SARIF uploads while default setup is enabled.
- `.github/workflows/release.yml` - Release: on a pushed `v*` tag, validates a leading-`v` SemVer tag, runs unit tests, decodes `KEYSTORE_BASE64` into `release.keystore`, builds signed release APKs with `-PenableAbiSplits=true`, renames them to `eBookSender-v<version>-universal.apk`, `eBookSender-v<version>-arm64-v8a.apk`, and `eBookSender-v<version>-armeabi-v7a.apk`, publishes them to GitHub Releases with the current version changelog section in English and Russian, then deploys `updates/latest.json` plus localized changelog markdown files to GitHub Pages for in-app update checks. Requires repo secrets `KEYSTORE_BASE64`, `RELEASE_STORE_PASSWORD`, `RELEASE_KEY_ALIAS`, `RELEASE_KEY_PASSWORD`; requires GitHub Pages configured for GitHub Actions deployment.
- `CHANGELOG.md` / `CHANGELOG.ru.md` - English fallback and Russian app update changelogs. Keep both updated for user-facing changes; the release workflow publishes them under `updates/changelog/` and the app update dialog extracts the matching version section.
- `DISCLAIMER.md` - project disclaimer covering warranty limits, user responsibility, content rights, third-party sources, FTP security, backups, and non-affiliation notes; README links to this document.
- `core/model/src/main/java/com/cybercat/ebooksender/model/update/AppUpdateManifest.kt` - serializable update manifest and artifact models consumed from the GitHub Pages `updates/latest.json` endpoint, including localized app changelog URLs.
- `core/model/src/main/java/com/cybercat/ebooksender/model/update/PocketBookServerUpdateManifest.kt` - serializable PocketBook `pb-ftp` server update manifest, artifact, and installed-version models consumed from the `pb-ftp` GitHub Pages manifest and `/version` endpoint, including optional `buildId` metadata for detecting rebuilt releases with the same SemVer and localized changelog URLs.
...
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/PocketBookServerUpdateManager.kt` - PocketBook `pb-ftp` server updater; checks automatically when a PocketBook profile connects and manually from settings, owns update state/status transitions, delegates manifest/version resolution to `PocketBookServerUpdateResolver`, and delegates FTP/control install workflow to `PocketBookServerUpdateInstaller`.
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/PocketBookServerUpdateResolver.kt` / `PocketBookServerUpdateInstaller.kt` / `PocketBookServerArtifactRepository.kt` / `PocketBookServerVersionReader.kt` - PocketBook-specific update helpers for `/version` reads, manifest validation, fixed launcher artifact selection, staging under `applications/.pb-ftp-update`, applying through local `POST /update`, legacy direct launcher replacement fallback, pending-restart completion after launcher delivery, and launcher artifact caching.
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/UpdateManifestLoader.kt` / `UpdateArtifactDownloader.kt` / `UpdateVerifiedArtifactLoader.kt` / `UpdateFileHelpers.kt` / `UpdateJobController.kt` / `UpdateStatusEvents.kt` - shared update infrastructure for HTTPS JSON manifest loading, artifact downloads with cancellation/progress, verified SHA-256 cache/download flow, safe cache file names, cache-directory cleanup, update check/install/status-clear job lifecycle, and status event ID handling used by both app and PocketBook server updaters.
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/UpdateChangelogLoader.kt` / `UpdateChangelogFormatter.kt` - shared update changelog download, caching, locale fallback, and Markdown version-section extraction for app and PocketBook server update dialogs.
...
- `core/model/src/main/java/com/cybercat/ebooksender/model/AppSettings.kt` / `CatalogFallbackNames.kt` - persisted user settings model, shared FTP mount/relative-root path normalization, folder/template preferences, stable catalog fallback path names, theme/haptics, localization, and local-device VPN-bypass behavior.
...
- `core/common/src/main/java/com/cybercat/ebooksender/util/SearchQueryNormalizer.kt` / `UrlHostMatcher.kt` - shared text normalization and URL host matching helpers used by OPDS search, source-title fallback, auth-source matching, and credentials lookup.
...
- `core/database/src/main/java/com/cybercat/ebooksender/data/database/dao/MangaSeriesBookmarkDao.kt` - Room DAO for saved/favorite/subscribed manga series; recovered moved-series bookmark replacement must stay inside `replaceRecoveredSeries(...)` so merge/upsert/delete remains transactional.
- `app/src/main/AndroidManifest.xml` / `app/src/main/res/xml/network_security_config.xml` - Android components, permissions, intent filters, services, backup policy, and default cleartext HTTP denial. PocketBook control HTTP is handled as an explicit app-code local/private-address exception in `PocketBookControlClient`.
...
- `app/src/main/java/com/cybercat/ebooksender/ui/EBookSenderApp.kt` - Compose app shell, theme, adaptive navigation scaffold, destination scroll state, and shared intent/toast handling.
- `app/src/main/java/com/cybercat/ebooksender/ui/AppRoutes.kt` - app-level route composables that wire Hilt ViewModels to feature screens for Send, Catalog, OPDS/Manga, and Settings.
- `app/src/main/java/com/cybercat/ebooksender/ui/UpdateOverlaysHost.kt` - app-level host for app/PocketBook-server update progress overlays and update dialogs.
- `core/ui/src/main/java/com/cybercat/ebooksender/ui/UpdateAvailableDialog.kt` - shared Material 3 update dialog shell with haptics and optional scrollable localized changelog section.
- `app/src/main/java/com/cybercat/ebooksender/ui/AppUpdateDialog.kt` - app-specific adapter for the shared update dialog shown when the app-open or manual update check finds a newer version.
...
- `app/src/main/java/com/cybercat/ebooksender/lifecycle/AppVisibilityTracker.kt` - process/activity visibility gate with listeners used by foreground services and notification cleanup to show transfer/download notifications only while the UI is backgrounded.
- `app/src/main/java/com/cybercat/ebooksender/notification/ForegroundServiceNotificationController.kt` - shared foreground-service notification helper for stable active progress notifications, completion notifications, and content intents.
- `app/src/main/java/com/cybercat/ebooksender/notification/AppNotificationVisibilityCleaner.kt` - app-wide notification cleanup hook that cancels tracked service notifications when the app becomes visible.
...
- `core/data/src/main/java/com/cybercat/ebooksender/data/catalog/PocketBookDatabaseReader.kt` - PocketBook `explorer-3.db` snapshot reader; caches the SQLite database files from the FTP mount root, reuses the local snapshot when remote size/mtime metadata is unchanged, opens the local copy read-only, and maps cursor rows under the Settings relative root to catalog file records.
- `core/data/src/main/java/com/cybercat/ebooksender/data/catalog/CatalogTreeBuilder.kt` - pure catalog tree builder for database records; filters supported file types, deduplicates PocketBook book records, maps metadata to `CatalogFile`, and groups Books/Documents/Manga with natural sorting through shared catalog grouping helpers.
- `core/data/src/main/java/com/cybercat/ebooksender/data/catalog/CatalogFolderScanner.kt` - generic FTP folder catalog source and PocketBook fallback when the SQLite database is unavailable or empty; reuses shared catalog group builders for fallback/folder/series groups.
- `core/data/src/main/java/com/cybercat/ebooksender/data/catalog/CatalogGrouping.kt` - shared catalog grouping helpers for DB-backed grouping and FTP-scanned fallback/folder/series group construction.
...
- `core/data/src/main/java/com/cybercat/ebooksender/data/opds/SearchOpdsCatalogUseCase.kt` - OPDS search interactor; loads book and Flibusta author results concurrently, emits partial results as each branch completes, preserves book-first ordering for the merged catalog, and bounds the whole search (including description/fallback requests) to 30 seconds. A failed or timed-out branch keeps available results with `isPartial`; authentication errors and caller cancellation remain explicit.
- `core/data/src/main/java/com/cybercat/ebooksender/data/opds/OpdsRepository.kt` - owns separate bounded catalog and OpenSearch-template caches (32 entries each, five-minute TTL). Cache cleanup, source removal, and credential changes invalidate both caches. Catalog and description parsing use `OpdsHttpClient.readDocument` for cancellable body reads.
- `feature/opds/src/main/java/com/cybercat/ebooksender/feature/opds/OpdsViewModel.kt` / `OpdsUiState.kt` - own a single replaceable catalog/search job. `isLoading` blocks navigation before results arrive; `isSearching` keeps the search indicator visible while partial results are usable. New searches clear paging, and `shouldShowPagination` exposes controls only for a loaded, settled catalog with another page to visit.
...
- `core/network/src/main/java/com/cybercat/ebooksender/data/opds/OpdsHttpClient.kt` - OPDS `HttpURLConnection` boundary; applies Accept/User-Agent headers, Basic auth from URLs or saved source credentials, manual redirects, timeouts, and HTTP status validation. `readDocument` owns a 20-second deadline across connection, redirects, and body parsing and disconnects on cancellation; publication downloads retain their existing connection/read timeouts.
- `core/network/src/main/java/com/cybercat/ebooksender/data/network/CancellableHttpConnection.kt` - shared OPDS/Com-X HTTP cancellation boundary. Wrap blocking connection, request and body operations in `runDisconnectingOnCancellation`; its separate IO watcher disconnects when cancellation starts, including while the operation is blocked or its progress callback suspends. Callers retain normal `finally` cleanup. Do not use completion-only Job callbacks to cancel blocking socket reads.
- `core/network/src/main/java/com/cybercat/ebooksender/data/manga/ComxArchiveUrlResolver.kt` - resolves Com-X archive authorization from reader/series/download IDs even when the parsed chapter has no direct `downloadUrl`. The chapter downloader must try the source adapter's archive method before page fallback; a missing parsed link does not establish archive unavailability.
...
- `core/data/src/main/java/com/cybercat/ebooksender/data/request/RequestCoordinator.kt` - shared foreground request coordinator primitive for single pending request handoff, optional active-request tracking, replacement/rejection submit policies, and buffered event delivery.
...
- `app/src/main/java/com/cybercat/ebooksender/transfer/TransferNotificationManager.kt` - transfer-specific notification text adapter backed by the shared foreground-service notification controller.
...
- `core/data/src/main/java/com/cybercat/ebooksender/transfer/FtpConnectionHealthMonitor.kt` - app-wide active-device health monitor started by `EBookSenderApplication`; periodically verifies the connected FTP device, skips checks during active transfers, and clears `ConnectionManager` when the server is no longer reachable.
- `app/src/main/java/com/cybercat/ebooksender/manga/MangaDownloadForegroundService.kt` - foreground manga chapter download service that keeps downloads running while the app is backgrounded, supports user cancellation, and adds fully completed chapters to the upload queue. Job-completion cleanup also covers cancellation before the coroutine starts, and stops the service independently of the original start ID because cancel intents advance it. Pending-request cancellation emits a terminal event to clear UI progress.
- `app/src/main/java/com/cybercat/ebooksender/manga/MangaDownloadNotificationManager.kt` - manga-specific notification text adapter backed by the shared foreground-service notification controller.
...
- `app/src/main/java/com/cybercat/ebooksender/transfer/UploadQueueManagerImpl.kt` - upload queue manager; coordinates queue state, persistence, metadata result application, upload path replanning, queue events, and cache cleanup while delegating file validation, metadata scheduling, source URI access, and queue storage to focused helpers.
- `app/src/main/java/com/cybercat/ebooksender/transfer/UploadItemValidator.kt` - app-local file admission helper for upload queue inserts; resolves display names/sizes, enforces supported extensions and max file size, and returns typed skip reasons for UI event rendering.
- `app/src/main/java/com/cybercat/ebooksender/transfer/MetadataExtractionDispatcher.kt` - bounded metadata-work dispatcher used by the upload queue; owns pending/running item sets, prioritization, worker signaling, lazy worker startup, idle worker shutdown, and parallelism control.
...
- `core/data/src/main/java/com/cybercat/ebooksender/data/network/LocalDeviceNetworkProvider.kt` / `LocalCleartextPolicy.kt` - Android network route helper and local cleartext guard for optional VPN bypass and PocketBook LAN-only control HTTP; private/local address validation must stay centralized here before any raw cleartext control socket is opened.
...
- `core/data/src/main/java/com/cybercat/ebooksender/data/manga/MangaArchiveHelper.kt` - packaging tool for creating CBZ/ZIP files from downloaded manga page images; unique output-file reservation uses coroutine `Mutex` because callers run in suspend download pipelines.
...
- `core/network/src/main/java/com/cybercat/ebooksender/data/manga/ComxMangaHttpClient.kt` - Com-X HTTP facade for text fetching and image downloads; delegates session, connection, Guard, and archive responsibilities to focused helpers, and raises browser-session refresh signals for Guard/forbidden responses that need WebView cookies refreshed. Text responses must be checked for Guard HTML/challenge URLs before HTTP status and auth-expiration handling: Com-X serves browser challenges with HTTP 404/403 too, and these must not clear login cookies.
...
- `core/network/src/test/java/com/cybercat/ebooksender/data/manga/ComxArchiveUrlResolverTest.kt`, `core/network/src/test/java/com/cybercat/ebooksender/data/network/CancellableHttpConnectionTest.kt`, and `core/data/src/test/java/com/cybercat/ebooksender/data/manga/MangaDownloadCoordinatorTest.kt` - archive resolution without direct links, stalled-request/suspending-callback cancellation, and pending manga request/event regressions. Run `:core:network:testDebugUnitTest :core:data:testDebugUnitTest` together with the existing OPDS HTTP tests when changing shared cancellation.
...
- `core/data/src/test/java/com/cybercat/ebooksender/data/opds/SearchOpdsCatalogUseCaseTest.kt`, `core/network/src/test/java/com/cybercat/ebooksender/data/opds/OpdsHttpClientTest.kt`, and `feature/opds/src/test/java/com/cybercat/ebooksender/feature/opds/OpdsPaginationStateTest.kt` - OPDS search concurrency, partial success, deadline/cancellation, auth, fallback, and pagination regressions. Run `:core:network:testDebugUnitTest :core:data:testDebugUnitTest :feature:opds:testDebugUnitTest`.
...
- Update `CHANGELOG.md` and `CHANGELOG.ru.md` whenever a user-facing change should appear in app update release notes.
