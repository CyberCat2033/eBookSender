...
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/UpdateManifestLoader.kt` / `UpdateArtifactDownloader.kt` / `UpdateVerifiedArtifactLoader.kt` / `UpdateFileHelpers.kt` / `UpdateJobController.kt` - shared update infrastructure for HTTPS JSON manifest loading, artifact downloads with cancellation/progress, verified SHA-256 cache/download flow, safe cache file names, cache-directory cleanup, and update check/install/status-clear job lifecycle used by both app and PocketBook server updaters.
