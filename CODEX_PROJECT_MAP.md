...
- `:core:database` - Room database, DAOs, entities, type converters, and the shared encrypted-secret table used for source/session secrets.
...
- `core/database/src/main/java/com/cybercat/ebooksender/data/database/entity/EncryptedSecretEntity.kt` / `dao/EncryptedSecretDao.kt` - shared Room table for encrypted app secrets keyed by namespace/owner/name; plaintext tokens, cookies, and credentials must not be stored here.
- `core/data/src/main/java/com/cybercat/ebooksender/data/security/EncryptedSecretStore.kt` - Android Keystore AES/GCM encrypted secret store backed by `EncryptedSecretDao`, using namespace/owner/name as AAD and deleting corrupted secrets on decrypt failure.
...
- `core/ui/src/main/java/com/cybercat/ebooksender/ui/SourceSelectionMenu.kt` - shared Material 3 dropdown source selector used by OPDS and Manga source pickers, with feature-provided haptics/actions.
...
- `app/src/main/java/com/cybercat/ebooksender/di/SecurityModule.kt` - Hilt binding for the Android Keystore encrypted-secret cipher.
...
- `core/network/src/main/java/com/cybercat/ebooksender/data/manga/MangaLibMangaAdapter.kt` - MangaLib source adapter (`sourceId = mangalib`); uses the public `api.cdnlibs.org` JSON API with `Site-Id: 1`, conservative download concurrency, and the existing browser-session refresh error path for DDoS-Guard/region/session challenges.
- `core/network/src/main/java/com/cybercat/ebooksender/data/manga/MangaLibMangaHttpClient.kt` / `MangaLibHttpConnectionFactory.kt` / `MangaLibMangaSessionManager.kt` - MangaLib HTTP/session boundary for API JSON loading, page image downloads from `img3.cdnlibs.org`, WebView cookie capture/restore, and encrypted durable cookie persistence through `MangaLibSessionSecretStore`.
- `core/network/src/main/java/com/cybercat/ebooksender/data/manga/MangaLibParser.kt` / `MangaLibParsingHelpers.kt` - MangaLib JSON parser and URL safety helpers for search results, series details, chapter lists, reader pages, trusted hosts, and stable chapter ids.
- `core/data/src/main/java/com/cybercat/ebooksender/data/manga/MangaLibEncryptedSessionSecretStore.kt` - `MangaLibSessionSecretStore` implementation that persists MangaLib session cookies through `EncryptedSecretStore`.
...
- `core/network/src/test/java/com/cybercat/ebooksender/data/manga/MangaLibParserTest.kt` - JVM unit tests for MangaLib JSON parsing and URL safety without live network or credentials.
- `core/data/src/test/java/com/cybercat/ebooksender/data/security/EncryptedSecretStoreTest.kt` - JVM unit tests for encrypted secret save/read/update/delete/owner cleanup/decrypt-failure cleanup with a fake cipher and DAO.
