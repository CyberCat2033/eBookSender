...
- `core/data/src/main/java/com/cybercat/ebooksender/data/update/UpdateManifestLoader.kt` / `UpdateArtifactDownloader.kt` / `UpdateFileHelpers.kt` - shared update infrastructure for HTTPS JSON manifest loading, artifact downloads with cancellation/progress, SHA-256 verification, safe cache file names, and cache-directory cleanup used by both app and PocketBook server updaters.
